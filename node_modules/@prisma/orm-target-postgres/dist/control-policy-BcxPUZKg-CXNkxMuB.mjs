import { d as isPostgresSchema } from "./postgres-schema-BfUzo1Yn-BLZvsxs9.mjs";
...
//#region ../../../3-targets/3-targets/postgres/dist/control-policy-BcxPUZKg.mjs
/**
...
function columnTypeLike(owner, identity) {
	if (identity.codecRef === void 0 || identity.codecBaseNativeType === void 0) throw new InternalError(`columnTypeLike: expected ${owner} carries no codec identity — the expected tree must be derived via contractToSchemaIR for planning`);
	return {
		...
		...ifDefined("typeParams", identity.codecRef.typeParams !== void 0 ? blindCast(identity.codecRef.typeParams) : void 0),
...
*/
function renderColumnDefaultSql(defaultNode, codecHooks) {
	const columnDefault = defaultNode.authored ?? defaultNode.resolved;
	if (columnDefault === void 0) return "";
...
const storageTypePlanCallStrategy = (issues, ctx) => {
	...
	for (const [typeName, typeInstance] of Object.entries(storageTypes).sort(([a], [b]) => a.localeCompare(b))) {
		const codecInstance = blindCast(typeInstance);
		...
		for (const op of planResult.operations) calls.push(new RawSqlCall(blindCast({
			...op,
			target: {
				id: op.target.id,
...
const notNullAddColumnCallStrategy = (issues, ctx) => {
	...
	const mutableCodecHooks = blindCast(ctx.codecHooks);
	const mutableStorageTypes = blindCast(ctx.storageTypes);
	for (const issue of issues) {
		if (issueOutcome(issue) !== "not-found") continue;
		const node = issueNode(issue);
		if (node === void 0 || node.nodeKind !== RelationalSchemaNodeKind.column) continue;
...
function classifyCall(call) {
	switch (call.factoryName) {
		...
		case "rawSql":
			if (blindCast(call).op?.target?.details?.objectType === "type") return "dep";
			return "alter";
...
function locationForCall(call) {
	...
	return Object.keys(location).length > 0 ? blindCast(location) : void 0;
}
...
function nativeEnumMemberChangeRefusal(options) {
	return `Native enum type "${options.ddlSchemaName}"."${options.typeName}" changed beyond appending new values (contract declares [${options.expectedMembers.join(", ")}], database has [${options.actualMembers.join(", ")}]). Prisma 8 does not modify a native enum's existing values (rename, removal, reorder) — see https://pris.ly/d/postgres-native-enums. Author the change manually with \`migration new\`.`;
}
/**
...
function mapColumnDefaultNodeIssue(issue, schemaName, tableName, columnName, codecHooks) {
	...
	const defaultSql = renderColumnDefaultSql(blindCast(issue.expected), codecHooks);
...
//# sourceMappingURL=control-policy-BcxPUZKg-CXNkxMuB.mjs.map