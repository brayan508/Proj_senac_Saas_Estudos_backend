...
//#region ../../../3-targets/6-adapters/postgres/dist/sql-renderer-BnTqIzZw.mjs
...
function renderTypedParam(index, codecId, codecDescriptorRegistry, many, typeParams, forceCast = false) {
	...
	if (forceCast || !POSTGRES_INFERRABLE_NATIVE_TYPES.has(nativeType) || many) return `$${index}::${nativeType}${arraySuffix}`;
	return `$${index}`;
}
...
function renderProjection(projection, contract, pim) {
	return projection.map((item) => {
		...
		if (item.expr.kind === "prepared-param-ref") return `${renderParamRef(item.expr, pim, true)} AS ${alias}`;
...
function renderParamRef(ref, pim, forceCast = false) {
	const index = pim.indexMap.get(ref);
	if (index === void 0) throw new InternalError("ParamRef not found in index map");
	if (ref.kind === "prepared-param-ref") return renderTypedParam(index, ref.codec.codecId, pim.codecDescriptorRegistry, ref.codec.many, ref.codec.typeParams, forceCast);
...
function renderInsert(ast, contract, pim) {
	...
	return `${(() => {
		...
		switch (action.kind) {
			case "do-nothing": return ` ON CONFLICT${target} DO NOTHING`;
			case "do-update-set": {
				...
				return ` ON CONFLICT${target} DO UPDATE SET ${updateEntries.map(([colName, value]) => {
...
//# sourceMappingURL=sql-renderer-BnTqIzZw-C8YDKp4u.mjs.map