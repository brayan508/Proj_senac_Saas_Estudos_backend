import { w as PG_TIMESTAMPTZ_STRING_CODEC_ID } from "./codec-ids-Cb0yO-IS--e0P2rfI.mjs";
import { n as buildPostgresCodecDescriptorRegistry } from "./codec-descriptor-BfoHQP3R-CMRVT-F6.mjs";
import { s as postgresError } from "./errors-DYyzTYzE-39CV_aMo.mjs";
import { n as postgresCodecRegistry, t as postgresCodecDescriptorRegistry } from "./registry-Bf5BvREu-DJ7g3REJ.mjs";
import { r as quoteIdentifier, t as escapeLiteral } from "./sql-utils-6j9YTLpr-BCyCBPBg.mjs";
import { a as jsonb, c as textArray, i as int8, l as timestamptz, n as buildSignMarkerBootstrapQueries, o as pgTable, r as int4, s as text, t as buildControlTableBootstrapQueries } from "./contract-free--ds6HUdk.mjs";
...
import { n as renderLoweredSql, t as adapterError } from "./sql-renderer-BnTqIzZw-C8YDKp4u.mjs";
...
import { errorMarkerRowCorrupt, rethrowMarkerReadError, withMarkerReadErrorHandling } from "@prisma/orm-framework/errors/execution";
...
//#region ../../../3-targets/6-adapters/postgres/dist/control-adapter-C2Xw0sJP.mjs
...
function markerRowDecodeWhy(detail) {
	return `Invalid contract marker row: ${detail}`;
}
function decodePostgresMarkerRow(row, space) {
	if (typeof row !== "object" || row === null) {
		const cause = /* @__PURE__ */ new TypeError(`expected object marker row, got ${typeof row}`);
	...
	try {
		...
		throw errorMarkerRowCorrupt({
			why: markerRowDecodeWhy(error instanceof Error ? error.message : String(error)),
			space,
			markerLocation: POSTGRES_MARKER_TABLE,
			cause: error
		});
	...
	*/
	...
	async readMarkerResult(driver, space) {
		...
		try {
			return {
				kind: "present",
				record: parseContractMarkerRow(decodePostgresMarkerRow(row, space))
			};
		} catch (error) {
			rethrowMarkerReadError(error, {
				space,
...
function parsePgNameArray(value) {
	if (Array.isArray(value)) throw new TypeError(`expected raw text for a Postgres array, got ${typeof value}`);
	if (typeof value !== "string") return [];
	return parsePostgresListText(value).map((entry) => String(entry));
}
/**
...
function unquoteIdentifier(segment) {
	return segment.length >= 2 && segment.startsWith("\"") && segment.endsWith("\"") ? segment.slice(1, -1).replaceAll("\"\"", "\"") : segment;
}
...
*/
function isPgReferentialActionRule(rule) {
	return Object.hasOwn(PG_REFERENTIAL_ACTION_MAP, rule);
}
function mapReferentialAction(rule) {
	if (!isPgReferentialActionRule(rule)) throw adapterError("CONTRACT.INTROSPECTION_UNSUPPORTED", `Unknown PostgreSQL referential action rule: "${rule}". Expected one of: NO ACTION, RESTRICT, CASCADE, SET NULL, SET DEFAULT.`, { meta: { rule } });
...
function parsePgReloptions(reloptions, indexName) {
	if (Array.isArray(reloptions)) throw new TypeError(`expected raw text for a Postgres array, got ${typeof reloptions}`);
	if (reloptions === null || reloptions === void 0) return;
	if (typeof reloptions !== "string") return;
	const entries = parsePostgresListText(reloptions).map((entry) => String(entry));
...
const INTROSPECTION_OUTPUT_SETTINGS = {
	timeZone: "UTC",
	dateStyle: "ISO, MDY",
	intervalStyle: "postgres"
};
async function readOutputSettings(driver) {
	const { rows } = await driver.query(`SELECT current_setting('TimeZone') AS "timeZone",
            current_setting('DateStyle') AS "dateStyle",
            current_setting('IntervalStyle') AS "intervalStyle"`);
	return rows[0];
}
async function applyOutputSettings(driver, settings, local) {
	await driver.query(`SELECT set_config('TimeZone', $1, $4),
            set_config('DateStyle', $2, $4),
            set_config('IntervalStyle', $3, $4)`, [
		settings.timeZone,
		settings.dateStyle,
		settings.intervalStyle,
		local
	]);
}
function sameOutputSettings(a, b) {
	return a?.timeZone === b.timeZone && a.dateStyle === b.dateStyle && a.intervalStyle === b.intervalStyle;
}
/**
...
*/
async function readWithDefaultOutputSettings(driver, read) {
	const callerSettings = await readOutputSettings(driver);
	if (callerSettings === void 0) return read();
...
async function pgRenderDdlColumnDefault(def, nativeType, codecLookup, codecRef) {
	...
	if (Array.isArray(def.value) && nativeType.endsWith("[]")) return `DEFAULT ${renderDefaultLiteral(def.value, {
		many: true,
		nativeType
	})}`;
	if (codecRef !== void 0) {
		const descriptor = codecLookup.descriptorFor?.(codecRef.codecId);
		const codec = descriptor === void 0 ? codecLookup.get(codecRef.codecId) : materializeCodec(descriptor, codecRef, { name: codecRef.codecId });
		if (codec !== void 0) {
...
async function pgRenderAlterTable(node, codecLookup) {
	...
	return {
		sql: `ALTER TABLE ${tableRef} ${(await Promise.all(node.actions.map((action) => action.accept(actionVisitor)))).join(", ")}`,
		params: []
	};
}
...
//# sourceMappingURL=control-adapter-C2Xw0sJP-ST9vPuLe.mjs.map