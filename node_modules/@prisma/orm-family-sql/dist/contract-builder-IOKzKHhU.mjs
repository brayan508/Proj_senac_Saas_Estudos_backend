...
import { d as toStorageTypeInstance } from "./types-D0B65hip.mjs";
import { n as checkSqlDefaultBody, o as reservedSqlDefaultBody } from "./validators-CR2v1106.mjs";
import { i as toOneNullabilityContradictionMessage, n as buildSqlContractFromDefinition, r as contractError } from "./derived-checks-6WZKIUxH-CWip2UN5.mjs";
...
import { bindEnumType, createEntityHelpersFromNamespace, enumType, isEnumTypeHandle, member, resolveToOneRelationNullable } from "@prisma/orm-framework/contract-authoring";
import { canonicalizeTaggedLiteralBody, describeTaggedLiteralFailure, resolveTemplateTagEscapes } from "@prisma/orm-framework/components/control";
...
var ContractModelBuilder = class ContractModelBuilder {
	...
	attributes(specOrFactory) {
		return blindCast(new ContractModelBuilder(this.stageOne, specOrFactory, this.sqlFactory, this.spaceId, this.tableName));
	}
	sql(specOrFactory) {
...
function belongsToNullable(relationName, declaredNullable, spec, fieldNames) {
	const { nullable, contradiction } = resolveToOneRelationNullable({
		declaredNullable,
		localFieldNullability: fieldNames.map((fieldName) => spec.fieldBuilders[fieldName]?.build().nullable === true),
		ownsReference: true
	});
	if (contradiction !== void 0) throw contractError("CONTRACT.RELATION_INVALID", toOneNullabilityContradictionMessage(`Relation "${spec.modelName}.${relationName}"`, contradiction), { meta: {
		modelName: spec.modelName,
		relationName,
...
function lowerHasOwnershipRelation(relationName, relation, currentSpec, allSpecs) {
	...
	return {
		...
		toModel: targetModelName,
		toTable: targetSpec.tableName,
		...relation.kind === "hasMany" ? { cardinality: "1:N" } : {
			cardinality: "1:1",
			nullable: true
		},
		on: {
...
*/
function resolveDeferredColumns(spec, expression, fieldCodecIds) {
	...
	return fieldNames.map((fieldName, position) => {
		...
		if (name === void 0 || codecId === void 0) throw new InternalError(`Deferred index expression on "${spec.modelName}" resolved no column for field "${fieldName}"`);
		return {
			name,
			codecId
		};
...
function resolveModelNode(spec, allSpecs, storageTypes, storageTypeReverseLookup, extensions) {
	...
	const indexes = (spec.sqlSpec?.indexes ?? []).map((index) => {
		...
		return index.expression !== void 0 ? {
			...carried,
			expression: typeof index.expression === "string" ? index.expression : index.expression.render(resolveDeferredColumns(spec, index.expression, fieldCodecIds))
		} : {
			...carried,
...
function sql(strings, ...values) {
	if (values.length > 0) throw contractError("CONTRACT.DEFAULT_SQL_INTERPOLATION", "sql`...` does not support interpolation; write the SQL as one literal.", { meta: { interpolations: values.length } });
	const canonical = canonicalizeTaggedLiteralBody(resolveTemplateTagEscapes(strings.raw.join("")));
	if (!canonical.ok) throw contractError("CONTRACT.DEFAULT_INVALID", describeTaggedLiteralFailure(canonical.reason), { meta: {
		reason: canonical.reason,
		offset: canonical.offset
	} });
	const reserved = reservedSqlDefaultBody(canonical.body);
	if (reserved !== void 0) throw contractError("CONTRACT.DEFAULT_INVALID", `Write .default(${reserved}()) instead of sql\`${reserved}()\`; ${reserved}() is a Prisma default function, not raw SQL.`, { meta: {
		reason: "reserved-function",
		expression: canonical.body
	} });
	const rejected = checkSqlDefaultBody(canonical.body);
	if (rejected !== void 0) throw contractError("CONTRACT.DEFAULT_INVALID", rejected, { meta: {
		reason: "unsafe-sql",
		expression: canonical.body
	} });
	return {
...
//#endregion
export { check as a, extensionModel as c, model as d, now as f, buildContractDefinition as i, field as l, sql as m, bindEnumType as n, defineContract as o, rel as p, buildBoundContract as r, enumType as s, autoincrement as t, member as u };

//# sourceMappingURL=contract-builder-IOKzKHhU.mjs.map