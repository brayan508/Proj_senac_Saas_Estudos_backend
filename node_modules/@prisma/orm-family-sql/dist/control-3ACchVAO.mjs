...
import { u as isStorageTypeInstance } from "./types-D0B65hip.mjs";
import { n as checkSqlDefaultBody, o as reservedSqlDefaultBody } from "./validators-CR2v1106.mjs";
import { c as SqlSchemaIR, n as RelationalSchemaNodeKind, u as SqlTableIR } from "./types-DvrIPGUN-wxavfzyJ.mjs";
import { c as sqlAttributeSpecs } from "./sql-attribute-specs-BjCzXYcO-Cj8SS3iE.mjs";
import { t as sqlEmission } from "./dist-D5__KJs-.mjs";
import { i as sqlFamilyPslBlockDescriptors, n as sqlFamilyAuthoringTypes, r as sqlFamilyEntityTypes, t as sqlFamilyAuthoringFieldPresets } from "./authoring-type-constructors-C1ogrr_x-B9lfq5gm.mjs";
import { n as classifyDiffSubjectGranularity, o as extractCodecControlHooks, s as verifySqlSchemaByDiff, t as classifyDiffEntityKind } from "./schema-verify-CrvQZ5V8-BfrNHJbX.mjs";
import { t as sqlFamilyError } from "./errors-B5g0xWro-mWH2C07F.mjs";
import { t as SqlContractSerializer } from "./sql-contract-serializer-0a4k494z-QYmio7Uz.mjs";
import { t as collectSupportedCodecTypeIds } from "./verify-BVk5bjdH-BXgGLM2U.mjs";
...
import { APP_SPACE_ID, SchemaTreeNode, VERIFY_CODE_HASH_MISMATCH, VERIFY_CODE_MARKER_MISSING, VERIFY_CODE_TARGET_MISMATCH, assembleAuthoringContributions, issueOutcome } from "@prisma/orm-framework/components/control";
...
function hasExecuteSteps(operation) {
	const execute = Reflect.get(operation, "execute");
	if (!Array.isArray(execute)) return false;
	return execute.every((step) => typeof step === "object" && step !== null && "sql" in step);
}
/**
...
function extractCodecTypeIdsFromContract(contract) {
	...
	if (typeof contract === "object" && contract !== null && "storage" in contract && typeof contract.storage === "object" && contract.storage !== null && "namespaces" in contract.storage && typeof contract.storage.namespaces === "object" && contract.storage.namespaces !== null) {
		const namespaces = blindCast(contract.storage.namespaces);
		for (const ns of Object.values(namespaces)) {
			...
			for (const table of Object.values(tbls)) if (typeof table === "object" && table !== null && "columns" in table && typeof table.columns === "object" && table.columns !== null) {
				const columns = blindCast(table.columns);
...
function createSqlFamilyInstance(stack) {
	...
	const target = blindCast(stack.target);
	const adapter = blindCast(stack.adapter);
	const extensions = blindCast(stack.extensions);
	...
	const targetSerializer = blindCast(target).contractSerializer;
...
function cloneRecord(value) {
	...
	return blindCast(Object.freeze({ ...value }));
}
...
function freezeDetailsValue(value) {
	...
	if (Array.isArray(value)) return blindCast(Object.freeze([...value]));
	return blindCast(Object.freeze({ ...value }));
}
...
const TABLE_NAME_CASE_CHANGED_CODE = "MIGRATION.TABLE_NAME_CASE_CHANGED";
function lowerFirst(value) {
	return value.charAt(0).toLowerCase() + value.slice(1);
}
function plannedTable(node, namespaceId) {
	return {
		namespaceId,
		tableName: node.name
	};
...
*/
function detectTableNameCaseChanges(input) {
	const dropped = [];
	const created = [];
	for (const issue of input.issues) {
		...
		(outcome === "not-expected" ? dropped : created).push(plannedTable(table, input.namespaceIdOf(issue)));
	}
	const conflicts = [];
	for (const create of created) {
		const drop = dropped.find((candidate) => candidate.namespaceId === create.namespaceId && candidate.tableName === lowerFirst(create.tableName));
		if (drop === void 0) continue;
		conflicts.push({
...
const PSL_INVALID_DEFAULT_SQL = "PSL_INVALID_DEFAULT_SQL";
/**
...
*/
function sqlDefaultLiteralTagEntry(tag) {
	return {
		written: {
			kind: "tag",
			tag
		...
		lower: ({ literal, context }) => {
			...
			if (reserved !== void 0) return reject(`Write @default(${reserved}()) instead of ${literal.tag}\`${reserved}()\`; ${reserved}() is a Prisma default function, not raw SQL.`);
			const reason = checkSqlDefaultBody(literal.body);
			if (reason !== void 0) return reject(reason);
			return {
				ok: true,
				value: {
					kind: "storage",
...
//#endregion
export { plannerSuccess as _, buildNativeTypeExpander as a, sqlDefaultLiteralTagEntry as b, controlPolicyForCall as c, detectDestructiveChanges as d, detectTableNameCaseChanges as f, plannerFailure as g, planFieldEventOperations as h, assembleAuthoringContributions as i, control_default as l, partitionIssuesByControlPolicy as m, PSL_INVALID_DEFAULT_SQL as n, contractNamespaceToSchemaIR as o, partitionCallsByControlPolicy as p, TABLE_NAME_CASE_CHANGED_CODE as r, contractToSchemaIR as s, INIT_ADDITIVE_POLICY as t, createMigrationPlan as u, runnerFailure as v, runnerSuccess as y };

//# sourceMappingURL=control-3ACchVAO.mjs.map