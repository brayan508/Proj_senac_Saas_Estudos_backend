...
import { r as lowerAuthoredIndex } from "./index-naming-Cj1Anq-T-w1JL7-EM.mjs";
import { t as lowerAuthoredCheck } from "./authored-check-naming-9UFx77db.mjs";
...
import { r as materializeForeignKeysAndIndexes } from "./foreign-key-materialization-B3HQzQIA.mjs";
...
import { c as isMaterializedSqlNamespace, d as toStorageTypeInstance, o as SqlStorage, s as applyFkDefaults } from "./types-D0B65hip.mjs";
import { p as validateStorageSemantics } from "./validators-CR2v1106.mjs";
...
import { mergeCapabilityMatrices, resolveToOneRelationNullable } from "@prisma/orm-framework/contract-authoring";
...
//#region ../../../2-sql/2-authoring/contract-ts/dist/derived-checks-6WZKIUxH.mjs
...
*/
function columnCodec(codecId, typeParams, codecLookup) {
	const descriptor = codecLookup?.descriptorFor?.(codecId);
	if (descriptor === void 0) return codecLookup?.get(codecId);
	return materializeCodec(descriptor, {
		codecId,
		...ifDefined("typeParams", typeParams === void 0 ? void 0 : blindCast(typeParams))
	}, { name: codecId });
}
function encodeViaCodec(value, codec) {
	if (codec) return codec.encodeJson(value);
	return blindCast(value);
}
function encodeColumnDefault(defaultInput, codec, many = false) {
	if (defaultInput.kind === "function") return {
		kind: "function",
		expression: defaultInput.expression
	};
	if ("canonical" in defaultInput && defaultInput.canonical === true) return {
		kind: "literal",
		value: blindCast(defaultInput.value)
	};
	if (many) {
...
function checkMemberValues(handle, codecLookup) {
	const encoded = handle.values.map((value) => encodeViaCodec(value, codecLookup?.get(handle.codecId)));
	const values = [];
...
function buildStorageColumn(field, storageValueSetRef, codecLookup) {
	if (isValueObjectField(field)) {
		const encodedDefault = field.default !== void 0 ? encodeColumnDefault(field.default, codecLookup?.get(JSONB_CODEC_ID)) : void 0;
		return {
	...
	const encodedDefault = field.default !== void 0 ? encodeColumnDefault(field.default, columnCodec(codecId, field.descriptor.typeParams, codecLookup), field.many === true) : void 0;
...
function buildSqlContractFromDefinition(definition, codecLookup) {
	...
	for (const semanticModel of definition.models) {
		...
		for (const relation of semanticModel.relations ?? []) {
			...
			invariant(relation.toTable !== void 0, `Relation "${semanticModel.modelName}.${relation.fieldName}" is local but carries no target table; only cross-space relations may leave it unset.`);
...
//#endregion
export { toOneNullabilityContradictionMessage as i, buildSqlContractFromDefinition as n, contractError as r, applySqlSpecifierControlPolicy as t };

//# sourceMappingURL=derived-checks-6WZKIUxH-CWip2UN5.mjs.map