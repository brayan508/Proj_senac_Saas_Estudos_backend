...
import { A as PreparedParamRef, H as isQueryAst } from "./types-ZNKYVjLr-DzjvSHKB.mjs";
...
import { t as buildSqlAggregateDescriptorRegistry } from "./aggregate-descriptor-registry-DzThdoju.mjs";
import { t as buildCodecDescriptorRegistry } from "./codec-descriptor-registry-ECZ4ohtG.mjs";
...
import { AsyncIterableResult, RuntimeCore, checkAborted, checkMiddlewareCompatibility, raceAgainstAbort, runBeforeExecuteChain, runBeforeQueryChain, runExecuteWithMiddleware, runQueryWithMiddleware, runtimeError } from "@prisma/orm-framework/components/runtime";
...
//#region ../../../2-sql/5-runtime/dist/exports-n31xw-hZ.mjs
/**
...
function assertColumnCodecIntegrity(storage, codecDescriptors) {
	for (const [namespaceId, ns] of Object.entries(storage.namespaces)) for (const [tableName, table] of Object.entries(ns.entries.table ?? {})) for (const columnName of Object.keys(table.columns)) {
		...
		if (paramsSchema !== void 0 && ref.typeParams === void 0) {
...
function buildBindSiteParams(declaration) {
	...
	for (const [name, spec] of Object.entries(declaration)) {
		...
		params[name] = new BindSiteExpression(PreparedParamRef.of(name, codec, nullable), {
			codecId: codec.codecId,
			nullable
		});
	}
	return blindCast(Object.freeze(params));
}
/**
