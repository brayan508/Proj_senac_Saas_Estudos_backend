...
import "./derived-checks-DPdD01O0-DNBs1ZDA.mjs";
import { n as AuthoredIndexMethod } from "./index-naming-BNIXvg5R.mjs";
...
type UniqueConstraint<FieldNames extends readonly string[] = readonly string[], Name extends string | undefined = string | undefined> = {
...
type IndexExpressionInput = string | DeferredIndexExpression;
...
declare function createConstraintsDsl<IndexTypes extends IndexTypeMap = Record<never, never>>(): {
  ...
  id: {
    <FieldName extends string, const Name extends string | undefined = undefined>(field: ColumnRef<FieldName>, options?: NamedConstraintSpec<Name>): IdConstraint<readonly [FieldName], Name>;
    <FieldNames extends readonly string[], const Name extends string | undefined = undefined>(fields: { readonly [K in keyof FieldNames]: ColumnRef<FieldNames[K] & string>; }, options?: NamedConstraintSpec<Name>): IdConstraint<FieldNames, Name>;
  };
  unique: {
    <FieldName extends string, const Name extends string | undefined = undefined>(field: ColumnRef<FieldName>, options?: ConstraintOptions<Name>): UniqueConstraint<readonly [FieldName], Name>;
    <FieldNames extends readonly string[], const Name extends string | undefined = undefined>(fields: { readonly [K in keyof FieldNames]: ColumnRef<FieldNames[K] & string>; }, options?: ConstraintOptions<Name>): UniqueConstraint<FieldNames, Name>;
  };
  index: {
    <FieldNames extends readonly string[], const Name extends string | undefined = undefined>(fields: { readonly [K in keyof FieldNames]: ColumnRef<FieldNames[K] & string>; }, options?: IndexInput<Name, IndexTypes>): IndexConstraint<FieldNames, Name>;
    <const Name extends string | undefined = undefined>(options: ExpressionIndexInput<Name, IndexTypes>): IndexConstraint<never, Name>;
  };
  foreignKey: {
...
type AttributeUniques<AttributesSpec extends ModelAttributesSpec | undefined> = AttributesSpec extends {
...
type ModelNamedObjects<AttributesSpec extends ModelAttributesSpec | undefined, SqlSpec extends SqlStageSpec | undefined> = [...AttributeUniques<AttributesSpec>, ...SqlIndexes<SqlSpec>, ...SqlForeignKeys<SqlSpec>];
type InlineUniqueLiteralName<Field> = FieldStateOf<Field> extends {
  readonly unique: {
    readonly name?: infer Name;
  };
} ? StaticLiteralName<Name> : never;
type RelationForeignKeyLiteralName<Relation> = RelationStateOf<Relation> extends {
  readonly sql?: infer Spec;
} ? Spec extends {
  readonly fk?: {
    readonly name?: infer Name;
  };
} ? StaticLiteralName<Name> : never : never;
type ModelLiteralNamesBySource<Fields extends Record<string, ScalarFieldBuilder>, Relations extends Record<string, AnyRelationBuilder>, AttributesSpec extends ModelAttributesSpec | undefined, SqlSpec extends SqlStageSpec | undefined> = { readonly [FieldName in keyof Fields & string as `field:${FieldName}`]: InlineUniqueLiteralName<Fields[FieldName]>; } & { readonly [RelationName in keyof Relations & string as `relation:${RelationName}`]: RelationForeignKeyLiteralName<Relations[RelationName]>; } & {
...
type LiteralNamesInMoreThanOneSource<NamesBySource> = { readonly [Source in keyof NamesBySource]: NamesBySource[Source] & NamesBySource[Exclude<keyof NamesBySource, Source>]; }[keyof NamesBySource];
type HasDuplicateModelNames<Fields extends Record<string, ScalarFieldBuilder>, Relations extends Record<string, AnyRelationBuilder>, AttributesSpec extends ModelAttributesSpec | undefined, SqlSpec extends SqlStageSpec | undefined> = [DuplicateLiteralNames<ModelNamedObjects<AttributesSpec, SqlSpec>> | LiteralNamesInMoreThanOneSource<ModelLiteralNamesBySource<Fields, Relations, AttributesSpec, SqlSpec>>] extends [never] ? false : true;
type ValidateSqlStageSpec<Fields extends Record<string, ScalarFieldBuilder>, Relations extends Record<string, AnyRelationBuilder>, AttributesSpec extends ModelAttributesSpec | undefined, SqlSpec extends SqlStageSpec> = HasDuplicateModelNames<Fields, Relations, AttributesSpec, SqlSpec> extends true ? never : SqlSpec;
type ValidateAttributesStageSpec<Fields extends Record<string, ScalarFieldBuilder>, Relations extends Record<string, AnyRelationBuilder>, SqlSpec extends SqlStageSpec | undefined, AttributesSpec extends ModelAttributesSpec> = HasDuplicateModelNames<Fields, Relations, AttributesSpec, SqlSpec> extends true ? never : AttributesSpec;
type DuplicateModelNamesError = 'Error: two ids, uniques, indexes or foreign keys on this model have the same name';
type ModelDuplicateNames<Fields extends Record<string, ScalarFieldBuilder>, Relations extends Record<string, AnyRelationBuilder>, AttributesSpec extends ModelAttributesSpec | undefined, SqlSpec extends SqlStageSpec | undefined> = [AttributesSpec] extends [never] ? DuplicateModelNamesError : [SqlSpec] extends [never] ? DuplicateModelNamesError : HasDuplicateModelNames<Fields, Relations, AttributesSpec, SqlSpec> extends true ? DuplicateModelNamesError : undefined;
declare class ContractModelBuilder<ModelName extends string | undefined, Fields extends Record<string, ScalarFieldBuilder>, Relations extends Record<string, AnyRelationBuilder> = Record<never, never>, AttributesSpec extends ModelAttributesSpec | undefined = undefined, SqlSpec extends SqlStageSpec | undefined = undefined, IndexTypes extends IndexTypeMap = Record<never, never>, TSpaceId extends string = '<self>'> {
  ...
  readonly __duplicateNames: ModelDuplicateNames<Fields, Relations, AttributesSpec, SqlSpec>;
  ...
  attributes<const NextAttributesSpec extends ModelAttributesSpec>(specOrFactory: StageInput<AttributeContext<Fields>, NextAttributesSpec>): [ValidateAttributesStageSpec<Fields, Relations, SqlSpec, NextAttributesSpec>] extends [never] ? ContractModelBuilder<ModelName, Fields, Relations, never, SqlSpec, IndexTypes, TSpaceId> : ContractModelBuilder<ModelName, Fields, Relations, NextAttributesSpec, SqlSpec, IndexTypes, TSpaceId>;
  sql<const NextSqlSpec extends SqlStageSpec>(specOrFactory: StageInput<SqlContext<Fields, IndexTypes>, NextSqlSpec>): [ValidateSqlStageSpec<Fields, Relations, AttributesSpec, NextSqlSpec>] extends [never] ? ContractModelBuilder<ModelName, Fields, Relations, AttributesSpec, never, IndexTypes, TSpaceId> : ContractModelBuilder<ModelName, Fields, Relations, AttributesSpec, NextSqlSpec, IndexTypes, TSpaceId>;
...
type RelationStateOf<T> = T extends RelationBuilder<infer State> ? State : never;
 ...
 */
type AuthoredColumnDefaultLiteralValue = ColumnDefaultLiteralInputValue | bigint | readonly AuthoredColumnDefaultLiteralValue[];
type AuthoredColumnDefault = ColumnDefault | {
...
declare function buildContractDefinition(definition: LoweringInput): ContractDefinition;
//#endregion
//#region src/default-functions.d.ts
/** The database's current time, the same default PSL writes as `@default(now())`. */
declare function now(): ColumnDefault;
/** A database-assigned sequence value, the same default PSL writes as `@default(autoincrement())`. */
declare function autoincrement(): ColumnDefault;
//#endregion
//#region src/sql-default-literal.d.ts
/**
 ...
 */
declare function sql(strings: TemplateStringsArray, ...values: readonly never[]): ColumnDefault;
//#endregion
export { UniqueConstraintNode as A, field as B, MergeEnums as C, RelationNode as D, PrimaryKeyNode as E, buildSqlContractFromDefinition as F, sql as G, model as H, check as I, defineContract as L, bindEnumType as M, buildBoundContract as N, ScalarFieldBuilder as O, buildContractDefinition as P, enumType as R, IndexNode as S, ModelNode as T, now as U, member as V, rel as W, ExtractCodecTypesFromPack as _, CheckNode as a, IndexConstraint as b, ColumnRef as c, ContractInput as d, ContractModelBuilder as f, EnumTypeHandle as g, EnumMember as h, BoundEnumType as i, autoincrement as j, TargetFieldRef as k, ComposedAuthoringHelpers as l, DeferredIndexExpression as m, AuthoredColumnDefault as n, CodecInput as o, DeferredIndexColumn as p, AuthoredColumnDefaultLiteralValue as r, CodecTypeMap as s, AttachedEntities as t, ContractDefinition as u, FieldNode as v, ModelLike as w, IndexExpressionInput as x, ForeignKeyNode as y, extensionModel as z };
//# sourceMappingURL=contract-builder-tTGQ_MGg.d.mts.map