...
import { C as LiteralExpr, D as OrExpr, F as SubqueryExpr, I as TableSource, L as UpdateAst, O as OrderByItem, P as SelectAst, R as WindowFuncExpr, S as ListExpression, T as NullCheckExpr, U as isWhereExpr, V as isAggregateFn, _ as InsertAst, b as JsonArrayAggExpr, c as DeleteAst, f as EqColJoinOn, j as ProjectionItem, k as ParamRef, l as DerivedTableSource, n as AndExpr, o as ColumnRef, p as ExistsExpr, r as BinaryExpr, s as DefaultValueExpr, t as AggregateExpr, v as InsertOnConflict, w as NotExpr, x as JsonObjectExpr, y as JoinAst } from "./types-ZNKYVjLr-DzjvSHKB.mjs";
import { t as collectOrderedParamRefs } from "./util-DQQgv2j1-D6INZaRG.mjs";
import { n as JsonDocumentProjection, r as NativeJsonValueProjection, t as CodecJsonValueProjection } from "./ast-CEGv-zzk.mjs";
import { i as isExpression, o as toExpr } from "./expression-DqrE3myO.mjs";
...
function assertInsertConflictSkipCapability(contract, action, targeted) {
	requireCapability(contract, action, "insertOnConflictSkip");
	if (!targeted) requireCapability(contract, action, "insertOnConflictWithoutTarget");
}
function requireCapability(contract, action, capability) {
	if (hasContractCapability(contract, capability)) return;
	throw ormError("ORM.CAPABILITY_MISSING", `${action} requires contract capability "${capability}". Re-emit the contract against an adapter that reports it.`, { meta: {
		capability,
		action
	} });
}
function resolveInsertConflictColumns(contract, namespaceId, modelName, conflictOn, action) {
	...
	return conflictOn.map((fieldName) => {
		...
		if (column === void 0) throw ormError("ORM.ARGUMENT_INVALID", `${action} conflictOn field "${fieldName}" is not a scalar field of model "${modelName}"`, { meta: {
			action,
			model: modelName,
			field: fieldName
		} });
...
function createStorageRowMapper(contract, namespaceId, modelName) {
	const columnToField = getColumnToFieldMap(contract, namespaceId, modelName);
	return Object.keys(columnToField).length === 0 ? (row) => ({ ...row }) : (row) => mapColumnNames(row, columnToField);
}
function mapColumnNames(row, columnToField) {
	const mapped = {};
...
function mapPolymorphicRow(contract, namespaceId, baseModelName, polyInfo, row, variantName) {
	...
	if (!variant) return mapKnownColumnNames(row, getCompleteColumnToFieldMap(contract, namespaceId, baseModelName));
	const mtiTable = variant.strategy === "mti" ? variant.table : void 0;
	return mapKnownColumnNames(row, getMergedColumnToFieldMap(contract, namespaceId, baseModelName, variant.modelName, mtiTable));
}
function createPolymorphicRowMapper(contract, namespaceId, baseModelName, polyInfo, variantName) {
	const baseMapper = captureColumnMapper(() => getCompleteColumnToFieldMap(contract, namespaceId, baseModelName));
	const mappersByValue = /* @__PURE__ */ new Map();
	let pinnedMapper = baseMapper;
	for (const [name, variant] of polyInfo.variants) {
		if (variantName && name !== variantName) continue;
		const mapper = captureColumnMapper(() => getMergedColumnToFieldMap(contract, namespaceId, baseModelName, variant.modelName, variant.strategy === "mti" ? variant.table : void 0));
		if (variantName) pinnedMapper = mapper;
		for (const [value, candidate] of polyInfo.variantsByValue) if (candidate === variant) mappersByValue.set(value, mapper);
	}
	const discriminatorColumn = polyInfo.discriminatorColumn;
	return (row) => {
		const value = row[discriminatorColumn] ?? row["__prisma_polymorphic_discriminator"];
		return (variantName ? pinnedMapper : typeof value === "string" ? mappersByValue.get(value) ?? baseMapper : baseMapper)(row);
	};
}
function captureColumnMapper(resolve) {
	try {
...
function mapKnownColumnNames(row, columns) {
	const mapped = {};
...
function bindJoin(contract, join) {
	...
	return new JoinAst(join.joinType, bindFromSource(contract, join.source), join.on.kind === "eq-col-join-on" ? join.on : bindWhereExpr(contract, join.on, namespaceId), join.lateral);
}
...
function bindSelectAst(contract, ast) {
	...
	return new SelectAst({
		...
		where: ast.where ? bindWhereExpr(contract, ast.where, namespaceId) : void 0,
		...
		having: ast.having ? bindWhereExpr(contract, ast.having, namespaceId) : void 0,
		limit: ast.limit,
		offset: ast.offset,
...
function predicateComparison(op, left, right) {
	const nullable = (right.kind === "list" ? [left, ...right.values] : [left, right]).find((operand) => operand.kind === "prepared-param-ref" && operand.nullable);
	if (nullable?.kind !== "prepared-param-ref") return new BinaryExpr(op, left, right);
	if (op === "eq" || op === "neq") return new BinaryExpr(op === "eq" ? "isNotDistinctFrom" : "isDistinctFrom", left, right);
	throw ormError("ORM.FILTER_UNSUPPORTED", `Structured ORM ${op} comparisons do not support nullable prepared parameter "${nullable.name}"`, { meta: { parameter: nullable.name } });
}
function predicateExpression(value) {
	if (isExpression(value)) return value.buildAst();
}
function emptyState() {
	return {
		filters: [],
...
function listComparisonMethod(op) {
	return ((left, codec) => (values) => predicateComparison(op, left, paramList(codec, values)));
}
/**
...
function conflictSkipClause(tableName, conflictSkip) {
	if (!conflictSkip) return void 0;
	if (conflictSkip.columns.length === 0) return InsertOnConflict.doNothing();
	return InsertOnConflict.on(conflictSkip.columns.map((column) => ColumnRef.of(tableName, column))).doNothing();
}
function compileInsertReturning(contract, namespaceId, tableName, rows, returningColumns, conflictSkip) {
	const { rows: normalizedRows } = normalizeInsertRows(contract, namespaceId, tableName, rows);
	const ast = InsertAst.into(tableSourceForContract(contract, namespaceId, tableName)).withRows(normalizedRows).withOnConflict(conflictSkipClause(tableName, conflictSkip)).withReturning(buildReturningColumns(contract, namespaceId, tableName, returningColumns));
	const { params } = deriveParamsFromAst(ast);
	return buildOrmQueryPlan(contract, ast, params);
}
function compileInsertCount(contract, namespaceId, tableName, rows, conflictSkip) {
	const { rows: normalizedRows } = normalizeInsertRows(contract, namespaceId, tableName, rows);
	const ast = InsertAst.into(tableSourceForContract(contract, namespaceId, tableName)).withRows(normalizedRows).withOnConflict(conflictSkipClause(tableName, conflictSkip));
	const { params } = deriveParamsFromAst(ast);
	return buildOrmQueryPlan(contract, ast, params);
}
...
function compileInsertReturningSplit(contract, namespaceId, tableName, rows, returningColumns, conflictSkip) {
	if (rows.length === 0) throw ormError("ORM.MUTATION_DATA_MISSING", "create() requires at least one row", { meta: {
		method: "create",
		namespaceId,
		tableName
	} });
	return groupRowsByColumnSignature(rows).map((group) => compileInsertReturning(contract, namespaceId, tableName, group, returningColumns, conflictSkip));
}
function compileInsertCountSplit(contract, namespaceId, tableName, rows, conflictSkip) {
	...
	return groupRowsByColumnSignature(rows).map((group) => compileInsertCount(contract, namespaceId, tableName, group, conflictSkip));
}
...
function describeCollectionRows(options) {
	return {
		...describeExecutionRows(options),
		consume: createPreparedRowsConsumer(options)
	};
}
function describeExecutionRows(options) {
	...
	if (state.includes.length === 0) {
		...
		const mapper = polyInfo ? (rawRow) => blindCast(mapPolymorphicRow(contract, namespaceId, modelName, polyInfo, rawRow, state.variantName)) : (rawRow) => blindCast(mapStorageRowToModelFields(contract, namespaceId, modelName, rawRow));
		return {
			plan: compiled,
			consume: (rows) => mapResultRows(rows, mapper)
		};
	}
	return {
		plan: compileSelectWithIncludes(contract, context.aggregateDescriptors, namespaceId, tableName, state, modelName),
		consume: (rows) => consumeIncludeRows(context, state, namespaceId, modelName, rows)
	};
}
function deferResolution(resolve) {
	try {
		const value = resolve();
		return () => value;
	} catch (error) {
...
function createPreparedMapper(contract, namespaceId, modelName, variantName) {
	const resolved = deferResolution(() => {
		const polyInfo = resolvePolymorphismInfo(contract, namespaceId, modelName);
		return polyInfo ? createPolymorphicRowMapper(contract, namespaceId, modelName, polyInfo, variantName) : createStorageRowMapper(contract, namespaceId, modelName);
	});
	return (row) => resolved()(row);
}
function createPreparedRowsConsumer(options) {
	...
	const castRow = (row) => blindCast(row);
	if (includes.length === 0) return (rows) => mapResultRows(rows, (row) => castRow(mapRow(row)));
	return (rows) => new AsyncIterableResult((async function* () {
		const parents = (await rows.toArray()).map((raw) => ({
			raw,
			mapped: mapRow(raw)
		}));
...
function createPreparedIncludeConsumer(context, include) {
	const { contract } = context;
	const alias = include.relationName;
	if (include.scalar) return {
		alias,
		consume: createPreparedScalarConsumer(context, include, include.scalar)
	};
	if (include.combine) {
		const branches = Object.entries(include.combine).map(([name, branch]) => ({
			name,
			consume: branch.kind === "rows" ? createPreparedIncludeConsumer(context, {
				...include,
				nested: branch.state,
				scalar: void 0,
				combine: void 0
			}).consume : createPreparedScalarConsumer(context, include, branch.selector)
		}));
		return {
			alias,
			consume: (raw) => {
	...
	const mapRow = createPreparedMapper(contract, include.relatedNamespaceId, include.relatedModelName, include.nested.variantName);
	const bindings = deferResolution(() => {
		...
		const polyInfo = resolvePolymorphismInfo(contract, include.relatedNamespaceId, include.relatedModelName);
		for (const variant of polyInfo?.mtiVariants ?? []) for (const column of Object.keys(tables?.[variant.table]?.columns ?? {})) keys.add(`${variant.table}__${column}`);
		return new Map([...keys].map((key) => [key, deferResolution(() => resolveIncludedColumnBinding(contract, context, include, key))]));
	});
...
async function consumeFirstRow(rows) {
	return (await rows.toArray())[0] ?? null;
}
function describeCollectionFirst(options) {
	const rows = describeCollectionRows(options);
	return {
		...rows,
...
function dispatchCollectionRows(options) {
	...
	if (state.includes.length === 0) {
		const query = describeExecutionRows(descriptionOptions);
...
function consumeIncludeRows(context, state, namespaceId, modelName, rows) {
	const { contract } = context;
	const generator = async function* () {
...
function decodeIncludePayload(contract, context, include, raw, bindings) {
	...
	const polyInfo = resolvePolymorphismInfo(contract, include.relatedNamespaceId, include.relatedModelName);
	const mapChildRow = polyInfo ? (childRow) => mapPolymorphicRow(contract, include.relatedNamespaceId, include.relatedModelName, polyInfo, childRow, include.nested.variantName) : createStorageRowMapper(contract, include.relatedNamespaceId, include.relatedModelName);
	...
	return consumeRowInclude(rawChildren, mapChildRow, (key) => {
...
function decodeIncludedStorageRow(row, bindingFor) {
	const decoded = {};
	for (const [key, value] of Object.entries(row)) {
		...
		decoded[key] = binding ? decodeIncludedColumnValue(binding.ref, binding.codecId, binding.codec, value) : value;
	}
	return decoded;
}
function resolveIncludedColumnBinding(contract, context, include, key) {
	...
	return {
		ref,
		codec,
		codecId: context.codecDescriptors.codecRefForColumn(include.relatedNamespaceId, ref.table, ref.column)?.codecId ?? ref.storageColumn.codecId
	};
}
...
function decodeIncludedColumnValue(ref, codecId, codec, value) {
...
function resolveScalarIncludeMetadata(contract, context, include, scalar) {
	...
	return {
		resolved,
		codec: context.contractCodecs.forCodecRef(resolved.codec)
	};
}
function consumeScalarInclude(include, resolved, codec, raw) {
...
function shorthandToWhereExpr(context, namespaceId, modelName, filters) {
	...
	for (const [fieldName, value] of Object.entries(filters)) {
		...
		exprs.push(predicateComparison("eq", left, predicateExpression(value) ?? LiteralExpr.of(value)));
	}
...
var GroupedCollection = class GroupedCollection {
	...
	*/
	get prepared() {
		return { aggregate: (fn, configure) => this.#describeAggregate(fn, configure) };
	}
	...
	#describeAggregate(fn, configure) {
		...
		return {
			...
			async consume(source) {
				return (await source.toArray()).map((row) => {
...
function createHavingBuilder(contract, aggregates, namespaceId, modelName, tableName) {
	...
	for (const operation of aggregateOperationNames(aggregates)) builder[operation] = (field) => {
		...
		return createHavingComparisonMethods(new AggregateExpr(operation, field === void 0 ? void 0 : ColumnRef.of(tableName, fieldToColumn[field] ?? field)), () => resolveAggregate({
			aggregates,
			contract,
			namespaceId,
			tableName,
			fn: operation,
			column: field === void 0 ? void 0 : fieldToColumn[field] ?? field
		}).codec.codecId);
	};
	return blindCast(builder);
}
function createHavingComparisonMethods(metric, outputCodecId) {
	const buildBinaryExpr = (op, value) => {
		const expression = predicateExpression(value);
		if (expression !== void 0 && (expression.kind !== "prepared-param-ref" || expression.codec.codecId !== outputCodecId())) throw ormError("ORM.HAVING_EXPRESSION_UNSUPPORTED", "HAVING comparands require a prepared parameter with the aggregate output codec", { meta: { kind: expression.kind } });
...
function splitCreateArguments(optionsOrConfigure, configure) {
	if (typeof optionsOrConfigure === "function") return {
		options: void 0,
		configureCallback: optionsOrConfigure
	};
	return {
		options: optionsOrConfigure,
...
var CollectionImpl = class CollectionImpl {
	...
	*/
	...
	get prepared() {
		return {
			...
			all: (configure) => {
				return describeCollectionRows(this.#withAnnotationsFromMeta(configure, "all").#descriptionOptions());
			},
			first: (filter, configure) => {
				return describeCollectionFirst(this.#forFirst(filter, configure).#descriptionOptions());
			}
		};
	}
	#descriptionOptions() {
	...
	#forFirst(filter, configure) {
		return (filter === void 0 ? this : typeof filter === "function" ? this.where(filter) : this.where(filter)).limit(1).#withAnnotationsFromMeta(configure, "first");
	}
	...
	*/
	...
	#describeAggregate(fn, configure) {
		...
		return {
			plan: compiled,
			async consume(source) {
				const row = (await source.toArray())[0] ?? {};
				return blindCast(Object.fromEntries(results.map(({ alias, resolved, codec }) => {
					return [alias, (Object.hasOwn(row, alias) ? row[alias] : void 0) ?? emptyAggregateResult(resolved, codec)];
				})));
			}
		};
	}
	...
	*/
	createAll(data, optionsOrConfigure, configure) {
		...
		return this.#createAllWithAnnotations(data, this.#collectAnnotationsFromMeta(configureCallback, "write", "createAll"), conflictSkip);
	}
	#createAllWithAnnotations(data, annotationsMap, conflictSkip) {
		if (data.length === 0) {
			const generator = async function* () {};
			return new AsyncIterableResult(generator());
		}
		...
		if (this.contract.capabilities?.["sql"]?.["defaultInInsert"] !== true) {
			const plans = compileInsertReturningSplit(this.contract, this.namespaceId, this.tableName, mappedRows, selectedForInsert, conflictSkip).map((plan) => mergeAnnotations(plan, annotationsMap));
		...
		const compiled = mergeAnnotations(compileInsertReturning(this.contract, this.namespaceId, this.tableName, mappedRows, selectedForInsert, conflictSkip), annotationsMap);
	...
	#resolveConflictSkip(options, method) {
		if (options === void 0) return void 0;
		if (options.onConflict !== "skip") throw ormError("ORM.ARGUMENT_INVALID", `${method} onConflict must be "skip"; received ${JSON.stringify(options.onConflict)}`, { meta: {
			method,
			model: this.modelName
		} });
		...
		return { columns: resolveInsertConflictColumns(this.contract, this.namespaceId, this.modelName, conflictOn, method) };
	}
	...
	*/
	async createAndCount(data, optionsOrConfigure, configure) {
		const { options, configureCallback } = splitCreateArguments(optionsOrConfigure, configure);
		const conflictSkip = this.#resolveConflictSkip(options, "createAndCount()");
		if (data.length === 0) return 0;
		this.#assertNotMtiVariant("createAndCount()");
		const annotationsMap = this.#collectAnnotationsFromMeta(configureCallback, "write", "createAndCount");
		const rows = blindCast(data);
		const mappedRows = this.#mapCreateRows(rows);
		applyCreateDefaults(this.ctx, this.namespaceId, this.tableName, mappedRows);
		if (this.contract.capabilities?.["sql"]?.["defaultInInsert"] !== true) {
			const plans = compileInsertCountSplit(this.contract, this.namespaceId, this.tableName, mappedRows, conflictSkip).map((plan) => mergeAnnotations(plan, annotationsMap));
			let affectedRows = 0;
		...
		const compiled = mergeAnnotations(compileInsertCount(this.contract, this.namespaceId, this.tableName, mappedRows, conflictSkip), annotationsMap);
...
async function prepareQuery(runtime, declaration, callback) {
	let description;
	const statement = await runtime.prepare(declaration, (params) => {
	...
	return blindCast(description ? createPreparedRowQuery(description, statement) : statement);
}
function createPreparedRowQuery(description, statement) {
	return Object.freeze({ query(target, params, options) {
		return description.consume(statement.query(target, params, options));
	} });
}
//#endregion
export { Collection, GroupedCollection, all, and, createModelAccessor, createPreparedRowQuery, emptyState, not, or, orm, prepareQuery };
