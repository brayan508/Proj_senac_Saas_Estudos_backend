...
import { autoincrement, bindEnumType, buildBoundContract, buildSqlContractFromDefinition, check, field, member, model, now, rel, sql } from "@prisma/orm-family-sql/contract-ts/contract-builder";
import sqlFamilyPack from "@prisma/orm-family-sql/family/pack";
import { DEFAULT_FULL_TEXT_SEARCH_LANGUAGE, isFullTextIndexableCodec, renderFullTextIndexExpression } from "@prisma/orm-target-postgres/target/sql-utils";
...
function fullTextIndex(column, options) {
	...
	return {
		...
		expression: {
			...
			render: (columns) => {
				...
				invariant(resolved !== void 0, `fullTextIndex resolved no column for field "${column.fieldName}"`);
				if (!isFullTextIndexableCodec(resolved.codecId)) throw postgresError("CONTRACT.INDEX_INVALID", `fullTextIndex indexes a text column, but "${column.fieldName}" is stored as \`${resolved.codecId}\`.`, {
...
//#endregion
export { autoincrement, buildSqlContractFromDefinition, check, defineContract, enumType, field, fullTextIndex, member, model, nativeEnum, now, pg, policyAll, policyDelete, policyInsert, policySelect, policyUpdate, rel, rlsEnabled, role, sql };
